...
jags_model <- jags.model(
  file = "jags_tmp.txt", data = data, inits = inits, 
  n.chains = 3, n.adapt = 500
)
update(jags_model, n.iter = 500)
samples <- coda.samples(jags_model, variable.names = c("theta"), n.iter = 3334)
samples
...
subInfo = "Edited by msjung\n"
bannerBreak = "*********************************************************************\n"
cat(paste0(bannerBreak,bookInfo,subInfo, bannerBreak,"\n"))

#------------------------------------------------------------------------------
source("tools/common.R")
source("tools/mcmc_tools.R")

# Check that required packages are installed:
install_missing_packages(c("parallel","rjags","runjags","compute.es"))
...
nChainsDefault = calc_default_chain()
runjagsMethodDefault = calc_default_jags_method(nChainsDefault)
